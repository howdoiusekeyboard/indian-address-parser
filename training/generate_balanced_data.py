...
def generate_sample(
    ...
    use_house = "HOUSE_NUMBER" in required or (
        not use_plot and random.random() < weights.get("HOUSE_NUMBER", 0.75)
    )
...
def generate_balanced_test_set(
    n_samples: int = 150, min_per_entity: int = 20, seed: int = 42
) -> list[dict]:
    """
...
def main():
    ...
    parser.add_argument(
        "--min-per-entity", type=int, default=20, help="Minimum test samples per entity"
    )
